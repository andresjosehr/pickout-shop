...
{	
	...
	{		
		...
		if(is_numeric($item_id)){
			if ($res=Yii::app()->functions->getItemById($this->data['item_id'])){
				...
				if(is_numeric($row)){				
					if($resp=SingleAppClass::getCart($this->device_uiid)){
	...
	{		
		...
		if(!empty($device_id)){			
			...
			if ( $res = SingleAppClass::getCart($device_id)){
	...
	{		
		...
		if($res=SingleAppClass::getCart($this->device_uiid)){
	...
	{				
		...
		/*GET CART*/
		$res=SingleAppClass::getCart($this->device_uiid);
		...
		if($res):
		/*SET AUTO ADDRESS*/
		$cart_auto_address = getOption($this->merchant_id,'singleapp_cart_auto_address');
		if($cart_auto_address==1 && $this->data['transaction_type']=="delivery" && $res['delivery_fee']<=0 ){
			
			$token = isset($this->data['token'])?$this->data['token']:'';				
		    $client_info = SingleAppClass::getCustomerByToken($token);		
			...
			if($search_mode=="location"){
			   if(empty($res['delivery_lat'])){		 
				...
				if(empty($res['delivery_lat'])){
				   try {		   					   	
			   	      $resp_set_address = SingleAppClass::setAutoAddress( $this->merchant_id,
			   	      ...
			   	      //dump($resp_set_address);
			   	      $db->updateData("{{singleapp_cart}}",$resp_set_address,'cart_id',$res['cart_id']);
			   	      unset($db);
			   	      $auto_set_delivery_fee = $resp_set_address['delivery_fee'];
		...
		if($res){
			...
			if ($code==1){
    	       ...
    	       if (!FunctionsV3::isSearchByLocation()){
    	          ...
    	          $merchant_info = FunctionsV3::getMerchantInfo($this->merchant_id);    	          
    	          if($min_tables_enabled==1 && !empty($res['delivery_lat']) && $merchant_info){
    	          	   $merchant_lat = $merchant_info['latitude'];
		               $merchant_lng = $merchant_info['lontitude'];   
		               
		               $customer_location_lat = $res['delivery_lat'];
			           $customer_location_lng = $res['delivery_long'];
		               
		               $distance_unit=FunctionsV3::getMerchantDistanceType($this->merchant_id);
		               $distance_results_type = SingleAppClass::getDistanceResultsType($this->merchant_id);		               
		               
		               if($distance_results_type==1){
		               	  $distance_qty = SingleAppClass::getLocalDistance($distance_unit,
		               	  $customer_location_lat,$customer_location_lng,
		               	  $merchant_lat,$merchant_lng);
		               } else {
		               	  $distance_qty = FunctionsV3::getDistanceBetweenPlot(
		               ...
		               $distance_unit = $distance_unit=="M"?"miles":"kilometers";
		               if(!empty(FunctionsV3::$distance_type_result)){    	       	   	   	   	   	    
					       $distance_unit=FunctionsV3::$distance_type_result;
			           }	
			           
			           if($distance_unit=="ft" || $distance_unit=="meter" || $distance_unit=="mt"){
			           	  // do nothing
			           } else {
    	       ...
    	       if (FunctionsV3::hasModuleAddon("pointsprogram")){
    	       	  ...
    	       	  if ($earn_pts = SingleAppClass::getCartEarningPoints($cart,$subtotal,$this->merchant_id)){
    	       	  	 $db = new DbExt();
    	       	  	 $db->updateData("{{singleapp_cart}}",array(
	...
	{		
		...
		$day=Yii::app()->functions->getOption("stores_open_day",$this->merchant_id);
	...
	{			
		...
		if ( $res = SingleAppClass::getOpeningHours($this->merchant_id) ){
			$delivery_day = isset($this->data['delivery_date'])?date("l",strtotime($this->data['delivery_date'])):'';								
		...
		$singleapp_time_interval = getOption($this->merchant_id,'singleapp_time_interval');		
		if(empty($singleapp_time_interval)){
			$singleapp_time_interval=15;
		}
		$singleapp_time_format= getOption($this->merchant_id,'singleapp_time_format');
		if(empty($singleapp_time_format)){
			$singleapp_time_format = 12;
		}	
				
		$times = SingleAppClass::createTimeRange($time_start, $time_end , "$singleapp_time_interval mins",$singleapp_time_format);		
		if(is_array($times) && count($times)>=1){			
		} else {
			$times = SingleAppClass::createTimeRange("6:00", "23:59" , "$singleapp_time_interval mins",$singleapp_time_format);		
		}
	...
	public function actionsetDeliveryAddress()
	{
	    ...
	    try {
			...
			$resp = SingleAppClass::checkDeliveryAddresNew($this->merchant_id,$lat, $lng);
				...
				if(isset($this->data['save_address'])){
				  if($this->data['save_address']==1){
				  	 if(!empty($this->data['street']) && !empty($this->data['city'])){				  	 	
				  	 	if (!SingleAppClass::getBookAddressByClientID($client_id,$this->data['street'],$this->data['city'],$this->data['state'])){
			...
			if(!empty($params['search_address'])){
				if($res = SingleAppClass::getRecentLocationByID($this->device_uiid,$lat, $lng)){				
					$id = $res['id'];
					$DbExt->updateData("{{singleapp_recent_location}}",$params,'id',$id);
				} else {				
					$DbExt->insertData("{{singleapp_recent_location}}",$params);
				}		
			}
	...
	{
		$id = isset($this->data['addressbook_id'])?$this->data['addressbook_id']:'';
		if($id>0){
			if ( $res = Yii::app()->functions->getAddressBookByID($id)){
							    
			    $country_name = Yii::app()->functions->countryCodeToFull(isset($res['country_code'])?$res['country_code']:'');		
				...
				try {
					...
					$resp = SingleAppClass::checkDeliveryAddress($this->merchant_id,$res);
			        ...
			        if(is_array($resp) && count($resp)>=1){							
						if(isset($resp['delivery_fee'])){
							$params['delivery_fee']=$resp['delivery_fee'];
							$params['distance']=isset($resp['distance'])?$resp['distance']:0;
	...
	public function actiongetUserProfile()
	{
		...
		if($res = SingleAppClass::getCustomerByToken($token)){
			...
			$avatar = SingleAppClass::getAvatar($res['client_id']);
	...
	public function actionsavePushSettings()
	{
		...
		if($res = SingleAppClass::getCustomerByToken($token)){
			$client_id = $res['client_id'];		
			$params = array(
			  'enabled_push'=>isset($this->data['enabled_push'])?$this->data['enabled_push']:0,
			  'date_modified'=>FunctionsV3::dateNow(),
			  'ip_address'=>$_SERVER['REMOTE_ADDR']
			);
			if(empty($params['enabled_push'])){
				$params['enabled_push']=0;
			}
	...
	public function actionlogin()
	{
		...
		if ($res=SingleAppClass::appLogin($username,$password)){						
			...
			$params = array(			
			  ...
			  'device_platform'=>isset($this->data['device_platform'])?strtolower($this->data['device_platform']):'',
			  'device_id'=>isset($this->data['device_id'])?$this->data['device_id']:'',
			  'single_app_device_uiid'=>isset($this->data['device_uiid'])?$this->data['device_uiid']:'',
			  'single_app_merchant_id'=>$this->merchant_id,
			  'enabled_push'=>1
			);
	...
	{
		$this->actiongetAddressBookList(false);
	}
	
	public function actiongetAddressBookList($with_limit=true)
	{
		$token = isset($this->data['token'])?$this->data['token']:'';
		$page_action = isset($this->data['page_action'])?$this->data['page_action']:'';
	...
	public function actionsaveAddressBook()
	{
		...
		if($Validator->validate()){	
			...
			if ( $params['as_default']==2){
				$db->qry("UPDATE {{address_book}} SET as_default='1'  WHERE client_id=".FunctionsV3::q($client_id)." ");
			}
			...
			if($id>=1){
				...
				if(!SingleAppClass::checkAddressBook($client_id,$params['latitude'],$params['longitude'],$id)){
					if ( $db->updateData("{{address_book}}",$params,'id',$id)){
						$this->code = 1;
					    $this->msg = $this->t("Successfully updated");
					} else $this->msg = $this->t("Something went wrong cannot update records. please try again later");
	...
	public function actiongetOrderDetails()
	{
		...
		if($id>=1){
			$_GET['backend']='';
			if ($data = Yii::app()->functions->getOrder2($id) ){
	...
	public function actionreOrder()
	{
		...
		$id = isset($this->data['id'])?$this->data['id']:'';	
		if($id>=1){
			if ($res = Yii::app()->functions->getOrderInfo($id)){
				...
				if(SingleAppClass::getCart($this->device_uiid)){
	...
	public function actiongetAppSettings()
	{
		...
		SingleAppClass::clearCartParamaters($this->device_uiid);
