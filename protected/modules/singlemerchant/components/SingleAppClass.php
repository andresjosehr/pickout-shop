...
class SingleAppClass
{
    ...
    public static function getCart($device_id='')
    {
    	if(empty($device_id)){
    		return false;
    	}
    ...
    {        	
    	...
    	if($res=self::getCustomerByID($client_id)){    		    		
    		self::clearCart($res['device_id']);
    		self::clearCart($res['single_app_device_uiid']);
    	}
    }    
    ...
    public static function prettyBadge($status='')
	{
		...
		if($status=="pending"){
		   return '<span class="badge badge-primary">'.st($original_status).'</span>';
		} elseif ( $status=="process" ){
			return '<span class="badge badge-success">'.st($original_status).'</span>';
		} elseif ( preg_match("/properly set in/i", $status)){
			return '<span class="badge badge-danger">'.st($original_status).'</span>';
		} elseif ( preg_match("/caught/i", $status)){
			return '<span class="badge badge-danger">'.st($original_status).'</span>';	
		} elseif ( preg_match("/no/i", $status)){
			return '<span class="badge badge-danger">'.st($original_status).'</span>';		
		} elseif ( preg_match("/server/i", $status)){
			return '<span class="badge badge-danger">'.st($original_status).'</span>';			
		} else {			
		   return '<span class="badge badge-success">'.st($original_status).'</span>';
		}
